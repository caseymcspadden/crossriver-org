Astro = function()
{
	...
	Astro.prototype.julian_day_to_julian_date2 = function(jd)
	{
	...
	Astro.prototype.julian_day_to_julian_date = function(jd)
	{
  	with (Math) {
    	IJD = floor(jd);
			var secs = 86400*(jd - IJD);   // do we need tp add 0.5 to jd?
			var h = floor(secs/3600);
			var m = floor((secs-(h*3600))/60);
			var s = secs - h*3600 - m*60;
			var L = floor(IJD + 68569);
			var N = floor(4 * L / 146097);
			L = L - floor((146097*N + 3)/4);
			var I = floor(4000*(L + 1)/1461001);
			L = L - floor(1461 * I / 4) + 31;
			var J = floor(80 * L / 2447);
			var K = L - floor(2447 * J / 80);
			L = floor(J/11);
			J = J + 2 - 12*L;
			I = 100*(N - 49) + I + L;
		}
		return new Date(I,J,K,h,m,s);	
  }
	...
	Astro.prototype.deg_to_hms = function(deg)  // returns object {h:H, m:M, s:S}
	{
		...
		val = 60*(val-h);
		var m = Math.floor(val);
		var s = Math.round(100*60*(val-m))/100;
	...
	Astro.prototype.AltitudeAzimuthToRADec = function(coslat,sinlat,lst,alt,az)  // returns object {ra:RA, dec:DEC}
	{
		var ret = {};
	
		var sindec = sinlat * this.sind(alt) + coslat * this.cosd(alt) * this.cosd(az);
		var cosdec = Math.sqrt(1 - sindec*sindec);
		
		var cosH = (this.sind(alt) - sindec*sinlat)/(cosdec*coslat);
		
		ret.dec = this.asind(sindec);
		ret.ra = lst - this.acosd(cosH);
	...
	Astro.prototype.CalculatePrecessionMatrix = function(fromyear,toyear)  // returns a Matrix
	{
		var dMatrix = new Matrix();
		...
		var COSB=Math.cos(B) ;
		var COSC=Math.cos(C) ;
		dMatrix.set(0,0,COSA*COSB*COSC-SINA*SINB);
		dMatrix.set(0,1,-COSA*SINB-SINA*COSB*COSC);
		dMatrix.set(0,2,-COSB*SINC);
		dMatrix.set(1,0,SINA*COSB+COSA*SINB*COSC);
		dMatrix.set(1,1,COSA*COSB-SINA*SINB*COSC);
		dMatrix.set(1,2,-SINB*SINC);
		dMatrix.set(2,0,COSA*SINC);
		dMatrix.set(2,1,-SINA*SINC);
		dMatrix.set(2,2,COSC);
		return dMatrix;
	}
