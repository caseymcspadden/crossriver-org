...
class Services {
	...
	{		
		...
		$this->locationpath = "https://" . $headers["HOST"] . $options["docroot"] . "/services/" . $this->resource ."/";
	}
	...
	private function get_payments($conn,$args,$params)
	{
		$r = $this->resourcePath;
		...
		//curl_setopt($ch, CURLOPT_URL, $endpoint ."v1/payments/payment?sort_order=desc"); 
			...
			if (isset($arr['state']) && $arr['state']=='approved')
			{
				...
				if (isset($params['email']) && strlen($params['email'])>0)
				{
						$message = 'Thank you for your online payment of $'. $arr['transactions'][0]['amount']['total'] . " to Cross River Software.\r\n" .
									     ...
									     'You may visit www.crossriver.org at any time to view your invoice balances and payment history.';
						$headers = 'From: sales@crossriver.com' . "\r\n" .
											 'Reply-To: sales@crossriver.org';
